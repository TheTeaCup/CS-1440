...
public class TestShapeFun
{
    ...
    public void testSilent()
    {
        ...
        if (styleErrors == -1) {
            styleErrors = 0;
            printGrade(grade);
            fail(styleTest.getErrors());
        }
        grade = Math.max(4 - styleErrors, 0);
        if (styleErrors != 0) {
            printGrade(grade);
            fail("Checkstyle error:\n" + styleTest.getErrors());
        }
    }
 ...
 * @version Spring 2014
 */
class StyleTest extends AutomaticBean
{
    private static final String CHECKER_XML =
            "http://student2.cs.appstate.edu/dixoncs/JavaCodingStyle/"
                    + "cs_appstate2.xml";
    ...
    /**
     * Method finishLocalSetup.
     * Needed for AutomaticBean apparently.
     */
    @Override
    ...
    public int testCheckStyle(String filename)
    {
        ...
        // AuditListener listener = new DefaultLogger(sos,
